...
def obfuscate_CLRZ(real_query_d, p = 0.95, q = 0.2, m = 1):
    if q == 0:
        return real_query_d
    temp = real_query_d
    for i in range(m-1):
...
def obfuscate_OSSE(real_query_d, p = 0.95, q = 0.2, m = 1):
    if q == 0:
        return real_query_d
    temp = real_query_d
    for i in range(m-1):
        temp = np.hstack((temp,real_query_d))
    for i in range(len(temp)):
        coin_flips = np.random.rand(len(temp[0]))
        for j in range(len(temp[0])):
            if temp[i][j] == 1:
                if coin_flips[j] > p:
                    temp[i][j] = 0
...
def obfuscate_FROST(real_query_d, p = 0.95, q = 0.2, m = 1):
    if q == 0:
        return real_query_d
    temp = real_query_d
    for i in range(m-1):
...
def apply_defense(real_query_d, defense):
    if defense["method"] == "P":
        ...
        return obfuscate_CLRZ(real_query_d,defense["p"],defense["q"],defense["m"])
    else:
        return real_query_d
