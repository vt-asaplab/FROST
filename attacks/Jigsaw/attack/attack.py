...
class Attacker:
    ...
    def __init__(self,sim_kw_d,real_td_d,sim_F=None,real_F=None,\
        ...
        if countermeasure_params["alg"] == "clrz" or countermeasure_params["alg"] == "osse" or countermeasure_params["alg"] == "frost":
    ...
    def attack_step_3(self):
        ...
        while(len(self.unrec_td_set)>0):
            ...
            row_sum_M = M.sum(axis=1, keepdims=True)
            M = np.divide(M, row_sum_M, out=np.zeros_like(M), where=row_sum_M != 0)

            row_sum_M_ = M_.sum(axis=1, keepdims=True)
            M_ = np.divide(M_, row_sum_M_, out=np.zeros_like(M_), where=row_sum_M_ != 0)
            ...
            for i in range(len(top_td)):
                ...
                kw = np.argmin(np.linalg.norm(M[top_td[i]] - M_, axis=1))
